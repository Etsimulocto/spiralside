# ∴ Bloom Engine — 5 Git Bash Deploy Scripts

Run these in order from `~/spiralside` in Git Bash.

---

## BASH 1 — Write bloomengine.js
...
src = r"""// Nimbis anchor: js/app/views/bloomengine.js
// SPIRALSIDE -- BLOOM ENGINE v1.0
// Living fractal particle biome creator / scientific synthesizer control panel
// Pattern: flex:1 min-height:0 inner wrapper, canvas top 40%, scrollable panel bottom
...
function _beBuildHTML(container) {
  ...
  container.innerHTML =
    ...
    '<div id="be-wells-hint">&#9678; click canvas \u2192 attract well &middot; right-click \u2192 repel well</div>' +
...
## BASH 2 — Patch index.html (tab button + view div)
...
# --- tab button after tab-bloomslice ---
if 'tab-bloomengine' not in src:
    ...
    if m:
        ...
        print("ERROR: could not find tab-bloomslice anchor")
else:
    print("tab button already present")

# --- view div after view-bloomslice ---
if 'view-bloomengine' not in src:
    ...
    if m2:
        ...
        print("ERROR: could not find view-bloomslice anchor")
else:
...
## BASH 3 — Patch ui.js (viewInits + split mode loadPanel)
...
if not ui_path:
    print("ERROR: ui.js not found"); exit(1)

with open(ui_path, 'r', encoding='utf-8') as f:
    src = f.read()

# --- viewInits: add bloomengine after bloomslice entry ---
if 'bloomengine' not in src:
    m = re.search(r"bloomslice\s*:\s*\(\)\s*=>\s*window\.initBloomsliceView\s*&&\s*window\.initBloomsliceView\(\)[,]?", src)
    if m:
        insert = ",\n      bloomengine: () => window.initBloomEngineView && window.initBloomEngineView()"
        src = src[:m.end()] + insert + src[m.end():]
        print("viewInits entry inserted")
    else:
        print("WARNING: bloomslice viewInits anchor not found, trying spiral anchor")
        m2 = re.search(r"spiral\s*:\s*\(\)\s*=>\s*window\.initSpiralView\s*&&\s*window\.initSpiralView\(\)[,]?", src)
        if m2:
            insert = ",\n      bloomengine: () => window.initBloomEngineView && window.initBloomEngineView()"
            src = src[:m2.end()] + insert + src[m2.end():]
            print("viewInits entry inserted after spiral")
        else:
            print("ERROR: could not find viewInits anchor")
else:
    print("viewInits already has bloomengine")

# --- split mode _loadPanel I-block: add bloomengine after bloomslice ---
if "bloomengine" not in src or src.count("bloomengine") < 2:
    m3 = re.search(r"case\s*['\"]bloomslice['\"]", src)
    if m3:
        # find end of this case block (next case or closing brace)
        snippet = "\n      case 'bloomengine': window.initBloomEngineView && window.initBloomEngineView(); break;"
        # insert after the bloomslice case line end
        line_end = src.find('\n', m3.end())
        src = src[:line_end] + snippet + src[line_end:]
        print("split mode case inserted")
    else:
        print("WARNING: split mode bloomslice case not found, skip")
else:
    print("split mode already has bloomengine")
...
## BASH 4 — Patch main.js (import + global)
...
if not main_path:
    print("ERROR: main.js not found"); exit(1)
...
# --- import ---
if 'bloomengine' not in src:
    m = re.search(r'import\s*\{[^}]*initBloomslice[^}]*\}\s*from\s*["\'].*bloomslice\.js["\']', src)
    if m:
        insert = '\nimport { initBloomEngine } from "./views/bloomengine.js";'
        src = src[:m.end()] + insert + src[m.end():]
        print("import inserted after bloomslice import")
    else:
        # fallback: after particles import
        m2 = re.search(r'import\s*\{[^}]*initParticles[^}]*\}\s*from\s*["\'].*particles\.js["\']', src)
        if m2:
            insert = '\nimport { initBloomEngine } from "./views/bloomengine.js";'
            src = src[:m2.end()] + insert + src[m2.end():]
            print("import inserted after particles import")
        else:
...
if 'initBloomEngineView' not in src:
    m3 = re.search(r'window\.initBloomsliceView\s*=\s*initBloomslice', src)
    if m3:
        insert = '\nwindow.initBloomEngineView = initBloomEngine;'
        src = src[:m3.end()] + insert + src[m3.end():]
        print("global inserted after initBloomslice global")
    else:
        m4 = re.search(r'window\.initGuideView\s*=\s*initGuide', src)
        if m4:
            insert = '\nwindow.initBloomEngineView = initBloomEngine;'
            src = src[:m4.end()] + insert + src[m4.end():]
            print("global inserted after initGuide global")
        else:
            print("ERROR: could not find global anchor")
else:
...
## BASH 5 — Git commit + push

```bash
cd ~/spiralside && git add . && git commit -m "feat: add bloom engine tab - living fractal particle biome creator" && git push origin main --force
```

---

## Quick combos to try once live

| combo | effect |
|---|---|
| Lorenz Chaos + Vortex Spin | Strange attractor spirals |
| Spiral Arms + Polar Remap | Galaxy mode |
| Julia Field + Cosine Interfere | Alien geometry |
| Petal Count + Radial Symmetry | Mandalas |
| Heat Map ON | Population density heat overlay |
| Spectral Shift ON | Color tied to velocity |
| Click canvas | Drop attract well (pink) |
| Right-click canvas | Drop repel well (blue) |
| ◈ WHITE BLOOM | Reset to one white pixel |
