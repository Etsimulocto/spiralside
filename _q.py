...
PYTHON = r'C:/Users/quart/AppData/Local/Programs/Python/Python313/python.exe'

# ── 1. state.js — add quest to FAB_TABS ──────────────────────
f = open('C:/Users/quart/spiralside/js/app/state.js', 'r', encoding='utf-8')
src = f.read()
f.close()

OLD_STATE = "  { id: 'spiralcut', label: 'cut',  icon: '✂',  color: '#FF4BCB',\n    onOpen: () => window.initSpiralCutView && window.initSpiralCutView() },"
NEW_STATE = OLD_STATE + "\n  { id: 'quest',     label: 'quest', icon: '⚔',  color: '#FFD93D',\n    onOpen: () => window.initQuestView && window.initQuestView() },"

if OLD_STATE not in src:
    print('ERR: state.js anchor not found'); sys.exit(1)
if 'quest' in src:
    print('SKIP: quest already in state.js')
else:
    src = src.replace(OLD_STATE, NEW_STATE, 1)
    open('C:/Users/quart/spiralside/js/app/state.js', 'w', encoding='utf-8').write(src)
    print('OK state.js')

# ── 2. index.html — tab button + view div ────────────────────
f = open('C:/Users/quart/spiralside/index.html', 'r', encoding='utf-8')
src = f.read()
f.close()

# Tab button — insert after spiralcut button
OLD_TAB = '    <button class="tab-btn"        id="tab-spiralcut" onclick="switchView(\'spiralcut\')" >✂ cut</button>'
NEW_TAB = OLD_TAB + '\n    <button class="tab-btn"        id="tab-quest"     onclick="switchView(\'quest\')"     >⚔ quest</button>'

if OLD_TAB not in src:
    print('ERR: tab-spiralcut anchor not found'); sys.exit(1)
if 'tab-quest' in src:
    print('SKIP: tab-quest already in index.html')
else:
    src = src.replace(OLD_TAB, NEW_TAB, 1)
    print('OK index.html tab button')

# View div — insert before end of screen-app
OLD_VIEW = '</div><!-- end #screen-app -->'
NEW_VIEW = '  <div class="view" id="view-quest"></div>\n' + OLD_VIEW

if OLD_VIEW not in src:
    print('ERR: screen-app end anchor not found'); sys.exit(1)
if 'view-quest' in src:
    print('SKIP: view-quest already in index.html')
else:
    src = src.replace(OLD_VIEW, NEW_VIEW, 1)
    print('OK index.html view div')

open('C:/Users/quart/spiralside/index.html', 'w', encoding='utf-8').write(src)

# ── 3. js/app/views/quest.js — new module ────────────────────
import os
os.makedirs('C:/Users/quart/spiralside/js/app/views', exist_ok=True)
...
quest_path = 'C:/Users/quart/spiralside/js/app/views/quest.js'
if os.path.exists(quest_path):
    print('SKIP: quest.js already exists')
else:
    open(quest_path, 'w', encoding='utf-8').write(QUEST_JS)
    print('OK quest.js created')

# ── 4. ui.js — add quest to viewInits ────────────────────────
f = open('C:/Users/quart/spiralside/js/app/ui.js', 'r', encoding='utf-8')
src = f.read()
f.close()

# Find the spiralcut line in viewInits and add quest after it
OLD_UI = "spiralcut: () => window.initSpiralCutView  && window.initSpiralCutView(),"
NEW_UI = OLD_UI + "\n    quest:     () => window.initQuestView     && window.initQuestView(),"

if OLD_UI not in src:
    print('WARN: ui.js spiralcut viewInits anchor not found — searching for alternative...')
    # Try alternate spacing
    lines = src.split('\n')
    for i, line in enumerate(lines):
        if 'spiralcut' in line and 'initSpiralCutView' in line:
            print(f'  Found at line {i}: {repr(line[:80])}')
else:
    if 'initQuestView' in src:
        print('SKIP: quest already in ui.js viewInits')
    else:
        src = src.replace(OLD_UI, NEW_UI, 1)
        open('C:/Users/quart/spiralside/js/app/ui.js', 'w', encoding='utf-8').write(src)
        print('OK ui.js viewInits')

# ── 5. main.js — expose window.initQuestView ─────────────────
f = open('C:/Users/quart/spiralside/js/app/main.js', 'r', encoding='utf-8')
src = f.read()
f.close()

# Find the spiralcut exposure pattern and add quest after
OLD_MAIN = "window.initSpiralCutView = () => { import('./views/spiralcut.js').then(m => m.initSpiralCutView()); };"
NEW_MAIN = OLD_MAIN + "\nwindow.initQuestView     = () => { import('./views/quest.js').then(m => m.initQuestView()); };"

if OLD_MAIN not in src:
    print('WARN: main.js spiralcut anchor not found — searching...')
    lines = src.split('\n')
    for i, line in enumerate(lines):
        if 'spiralcut' in line.lower() and 'import' in line:
            print(f'  Found at line {i}: {repr(line[:100])}')
else:
    if 'initQuestView' in src:
        print('SKIP: quest already in main.js')
    else:
        src = src.replace(OLD_MAIN, NEW_MAIN, 1)
        open('C:/Users/quart/spiralside/js/app/main.js', 'w', encoding='utf-8').write(src)
        print('OK main.js')

print('\n=== ALL DONE ===')
print('Run: git add . && git commit -m "feat: add quest tab stub v1" && git push origin main --force')
