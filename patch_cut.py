#!/usr/bin/env python3
# ============================================================
# SPIRALSIDE — PATCH: SpiralCut view v0.1
# Run from ~/spiralside in Git Bash:
#   C:\Users\quart\AppData\Local\Programs\Python\Python313\python.exe patch_cut.py
#
# What this does:
#   1. Creates js/app/views/cut.js  (full SpiralCut module)
#   2. Patches js/app/main.js       (import + window global)
#   3. Patches js/app/ui.js         (viewInits entry)
...
with open('js/app/views/cut.js', 'w', encoding='utf-8') as f:
    f.write(CUT_JS)
print('✓ wrote js/app/views/cut.js')
...
if 'initCutView' in main_src:
    print('⚠ main.js already has initCutView — skipping main.js patch')
else:
    # Find the last import line and add cut import after it
    # Pattern: look for existing view imports to place after
    # We'll add after the last 'import {' block near the top
    # Use a safe anchor: look for initQuestView or another view import
    cut_import = "import { initCutView } from './views/cut.js';\n"

    # Find a good anchor — insert after any existing view import
    anchor_patterns = [
        "import { initQuestView }",
        "import { initGuideView }",
        "import { initLibrary }",
        "import { initVaultView }",
        "import { initForgeView }",
    ]
    inserted = False
    for pat in anchor_patterns:
        idx = main_src.find(pat)
        if idx != -1:
            # find end of that line
            eol = main_src.find('\n', idx)
            main_src = main_src[:eol+1] + cut_import + main_src[eol+1:]
            inserted = True
            print(f'✓ inserted cut import after "{pat[:40]}..."')
            break

    if not inserted:
        # Fallback: prepend after first import line
        first_import = main_src.find('\nimport ')
        if first_import != -1:
            eol = main_src.find('\n', first_import+1)
            main_src = main_src[:eol+1] + cut_import + main_src[eol+1:]
            print('✓ inserted cut import (fallback position)')
        else:
            print('✗ could not find import anchor in main.js — add manually:')
            print(f'  {cut_import.strip()}')

    # Wire window global — find where other window globals are exposed
    # Pattern: window.initXxxView = initXxxView;
    cut_global = "window.initCutView     = initCutView;\n"
    global_patterns = [
        "window.initQuestView",
        "window.initGuideView",
        "window.initLibrary",
        "window.initVaultView",
    ]
    ...
    for pat in global_patterns:
        ...
        if idx != -1:
            eol = main_src.find('\n', idx)
            main_src = main_src[:eol+1] + cut_global + main_src[eol+1:]
            inserted_global = True
            print(f'✓ inserted window.initCutView global')
            break
    if not inserted_global:
        print('⚠ could not auto-wire window.initCutView — add manually:')
        print(f'  {cut_global.strip()}')

    with open('js/app/main.js', 'w', encoding='utf-8') as f:
        f.write(main_src)
    print('✓ patched js/app/main.js')

# ── PATCH UI.JS ────────────────────────────────────────────
with open('js/app/ui.js', 'r', encoding='utf-8') as f:
    ui_src = f.read()

if 'spiralcut' in ui_src and 'initCutView' in ui_src:
    print('⚠ ui.js already has spiralcut/initCutView — skipping ui.js patch')
else:
    # Find viewInits object and add spiralcut entry
    # Pattern looks like:  'quest': () => window.initQuestView?.(),
    # or similar. We insert before the closing brace of viewInits.
    cut_init_line = "    'spiralcut': () => window.initCutView?.(),\n"

    # Try to find viewInits block
    vi_idx = ui_src.find('viewInits')
    if vi_idx == -1:
        vi_idx = ui_src.find('viewInit')

    if vi_idx != -1:
        # find the closing }; of viewInits
        # look for quest or another known entry to insert after
        quest_pat = "'quest'"
        guide_pat = "'guide'"
        anchor_found = False
        for pat in [quest_pat, guide_pat]:
            idx = ui_src.find(pat, vi_idx)
            if idx != -1:
                eol = ui_src.find('\n', idx)
                ui_src = ui_src[:eol+1] + cut_init_line + ui_src[eol+1:]
                anchor_found = True
                print(f'✓ inserted spiralcut viewInits entry after {pat}')
                break
        if not anchor_found:
            # find the closing brace
            close = ui_src.find('}', vi_idx + 50)
            if close != -1:
                ui_src = ui_src[:close] + cut_init_line + ui_src[close:]
                print('✓ inserted spiralcut viewInits entry (fallback)')
            else:
                print('✗ could not patch ui.js viewInits — add manually:')
                print(f"  {cut_init_line.strip()}")
    else:
        print('✗ could not find viewInits in ui.js — add manually:')
        print(f"  {cut_init_line.strip()}")

    with open('js/app/ui.js', 'w', encoding='utf-8') as f:
        f.write(ui_src)
    print('✓ patched js/app/ui.js')

# ── GIT COMMIT & PUSH ──────────────────────────────────────
print('\n--- git ---')
subprocess.run(['git', 'add', 'js/app/views/cut.js', 'js/app/main.js', 'js/app/ui.js'], check=True)
subprocess.run(['git', 'commit', '-m', 'feat: SpiralCut view v0.1 — clip storyboard timeline'], check=True)
subprocess.run(['git', 'push', '--force', 'origin', 'main'], check=True)
print('\n✅ Done! Vercel deploys in ~30s.')
print('   Open spiralside.com and tap the ✂ cut tab.')
