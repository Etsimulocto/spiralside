...
function _initSTT(){return !!(window.SpeechRecognition||window.webkitSpeechRecognition);}export function toggleMic(){if(!sttEnabled)return;const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){console.warn('[stt] not supported');return;}if(isRecording&&recognition){recognition.stop();isRecording=false;_updateMic();return;}recognition=new SR();recognition.continuous=false;recognition.interimResults=false;recognition.lang='en-US';recognition.onresult=e=>{const t=e.results[0][0].transcript;const inp=document.getElementById('msg-input');if(inp){inp.value=t;inp.style.height='auto';inp.style.height=Math.min(inp.scrollHeight,100)+'px';if(typeof window._sendMessage==='function'){setTimeout(window._sendMessage,120);}}};recognition.onend=()=>{isRecording=false;_updateMic();};recognition.onerror=e=>{console.warn('[stt] error:',e.error);isRecording=false;_updateMic();};try{recognition.start();isRecording=true;}catch(e){console.warn('[stt] start threw:',e);isRecording=false;}_updateMic();}export function speakReply(text){if(!ttsEnabled||!('speechSynthesis'in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.rate=0.95;u.pitch=1.0;window.speechSynthesis.speak(u);}
