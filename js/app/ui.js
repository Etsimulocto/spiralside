...
const _splitState = { a: 'chat', b: 'pi' };

export function toggleSplitMode() {
  _splitActive = !_splitActive;
  const btn = document.getElementById('tab-split');
  if (btn) btn.classList.toggle('active', _splitActive);
  if (_splitActive) _enableSplit();
  else _disableSplit();
}

function _enableSplit() {
  const app = document.getElementById('screen-app');
  if (!app) return;

  // Build two panel containers if not already there
  if (!document.getElementById('split-panel-a')) {
    const pa = document.createElement('div');
    pa.id = 'split-panel-a';
    const paTabs = _buildPanelTabbar('a');
    pa.appendChild(paTabs);
    app.appendChild(pa);

    const pb = document.createElement('div');
    pb.id = 'split-panel-b';
    const pbTabs = _buildPanelTabbar('b');
    pb.appendChild(pbTabs);
    app.appendChild(pb);
  }

  app.classList.add('split-mode');
  _loadPanelView('a', _splitState.a);
  _loadPanelView('b', _splitState.b);
}

function _disableSplit() {
  const app = document.getElementById('screen-app');
  if (!app) return;
  app.classList.remove('split-mode');
  // Move views back to app root
  ['a','b'].forEach(p => {
    const panel = document.getElementById('split-panel-' + p);
    if (!panel) return;
    panel.querySelectorAll('.view').forEach(v => app.appendChild(v));
    panel.remove();
  });
  // Restore active view
...
function _buildPanelTabbar(panel) {
  const bar = document.createElement('div');
  bar.className = 'split-panel-tabbar';
  bar.id = 'split-tabbar-' + panel;
  const TABS = ['chat','pi','imagine','forge','code','quest','codex','library','music','vault','studio','cut','style','store','account'];
  TABS.forEach(id => {
    const btn = document.createElement('button');
    btn.className = 'split-panel-tab' + (_splitState[panel] === id ? ' active' : '');
    btn.textContent = id;
    btn.onclick = () => _loadPanelView(panel, id);
    bar.appendChild(btn);
  });
  return bar;
}

function _loadPanelView(panel, id) {
  _splitState[panel] = id;
  const panelEl = document.getElementById('split-panel-' + panel);
  if (!panelEl) return;

  // Update tab highlight
  panelEl.querySelectorAll('.split-panel-tab').forEach(t => {
    t.classList.toggle('active', t.textContent === id);
  });

  // Move view into panel, deactivate others in this panel
  const app = document.getElementById('screen-app');
  const viewEl = document.getElementById('view-' + id);
  if (!viewEl) return;

  // Move all views out of panels back to app first
  panelEl.querySelectorAll('.view').forEach(v => {
    v.classList.remove('active');
    app.appendChild(v);
  });

  // Move target view into panel and activate
