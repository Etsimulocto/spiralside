...
// Saves current DOM tab order to IDB
async function saveTabOrder() {
  const tabs  = [...document.querySelectorAll('#tab-bar .tab-btn')];
  const order = tabs.map(t => t.id.replace('tab-', ''));  // ['chat','forge',...]
  try {
    await dbSet('config', { key: 'tab_order', data: order });
  } catch(e) { console.warn('[tabOrder] save failed:', e); }
}

// Restores tab order from IDB — call once on app init
export async function restoreTabOrder() {
  try {
    const rec   = await dbGet('config', 'tab_order');
    const order = rec?.data;                              // array of tab IDs
...
// Wires drag-to-reorder on the tab bar — call once on app init
export function initTabDrag() {
  const bar = document.getElementById('tab-bar');
  if (!bar) return;

  let dragging = null;  // the tab btn currently being dragged

  // Make all current + future tab btns draggable
  function makeTabsDraggable() {
    bar.querySelectorAll('.tab-btn').forEach(btn => {
      if (btn.dataset.draggable) return;   // already wired
      btn.dataset.draggable = '1';
      btn.draggable = true;

      btn.addEventListener('dragstart', e => {
        dragging = btn;
        btn.style.opacity = '0.4';         // visual: dragged item fades
        e.dataTransfer.effectAllowed = 'move';
      });

      btn.addEventListener('dragend', () => {
        btn.style.opacity = '';            // restore opacity
        bar.querySelectorAll('.tab-btn').forEach(b => b.style.borderLeft = '');
        dragging = null;
        saveTabOrder();                    // persist new order to IDB
      });
    });
  }

  // Drop zone on the bar itself — handles reorder
  bar.addEventListener('dragover', e => {
    ...
    bar.querySelectorAll('.tab-btn').forEach(b => b.style.borderLeft = '');
    target.style.borderLeft = '2px solid #00F6D6';
    // Reorder DOM: insert dragging before target
    const kids  = [...bar.querySelectorAll('.tab-btn')];
    const fromI = kids.indexOf(dragging);
    const toI   = kids.indexOf(target);
    if (fromI < toI) bar.insertBefore(dragging, target.nextSibling);
    else             bar.insertBefore(dragging, target);
  });
  ...
  makeTabsDraggable();   // wire existing tabs
}
