# Writes spiral.js directly -- no Supabase, no escaping issues
# Run from ~/spiralside

JS = r"""// Nimbis anchor: js/app/views/spiral.js
...
with open('js/app/views/spiral.js', 'w', encoding='utf-8') as f:
    f.write(JS)
print('wrote spiral.js (' + str(len(JS)) + ' chars)')
...
run('git add js/app/views/spiral.js')
run('git commit -m "fix: rewrite spiral.js with raw string, no escaping issues"')
