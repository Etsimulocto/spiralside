...
# ── Write sync.js ──
sync = open("C:/Users/quart/spiralside/js/app/sync.js", "w", encoding="utf-8")
sync.write("""// ============================================================
// SPIRALSIDE — SYNC v1.0
// Supabase cloud backup for all user data
// Mirrors IDB/localStorage to user_data table on every save
// Pulls from cloud on load if IDB is empty (new device/wipe)
...
export async function syncLoad(record_type) {
  const token = state.session?.access_token;
  try {
    const r = await fetch(SUPA_URL + '/rest/v1/user_data?user_id=eq.' + state.user.id + '&record_type=eq.' + record_type + '&select=data', {
...
export async function syncLoadAll() {
  const token = state.session?.access_token;
  try {
    const r = await fetch(SUPA_URL + '/rest/v1/user_data?user_id=eq.' + state.user.id + '&select=record_type,data,updated_at', {
      headers: { 'Authorization': 'Bearer ' + token, 'apikey': SUPA_KEY },
    });
    return await r.json() || [];
